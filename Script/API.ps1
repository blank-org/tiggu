...
function CompressHtml($iRoot, $i, $oRoot, $o) {
	java -jar $Env:TIGGU\Tools\HTML-Compressor.jar -t html --compress-js --js-compressor closure --closure-opt-level simple --compress-css -o $oRoot$o $iRoot$i
	Status $?
}
...
function CompressCss($iRoot, $i, $oRoot, $o) {
	java -jar $Env:TIGGU\Tools\Closure-StyleSheets.jar --output-file $oRoot$o $iRoot$i
	Status $?
}
